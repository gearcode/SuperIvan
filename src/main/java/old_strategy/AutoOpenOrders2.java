 ...
 * 双向格子开仓
 * 当盈利到达止盈金额时平仓并停止策略
 * 当单向开单数达到止盈格数时平仓并停止策略
 * @author liteng
 *
 */
public class AutoOpenOrders2 implements IStrategy {
	...
	public double stop_profit = 100.00;
	...
	GridCallback gridsNormalCallbackAsk = new GridCallback() {
		...
		public void onGridChange(Grid leave, Grid enter, double price) {
			...
			if(leave.getOffset() < enter.getOffset()) {
				 ...
				 * target grids count
				 */
				if(gridsNormalAsk.getAllOrders().size() + 1 >= stop_orders) {
					closeAllAndStop(1);
					return;
				}
	...
	GridCallback gridsNormalCallbackBid = new GridCallback() {
		...
		public void onGridChange(Grid leave, Grid enter, double price) {
			...
			if(leave.getOffset() > enter.getOffset()) {
				 ...
				 * target grids count
				 */
				if(gridsNormalBid.getAllOrders().size() + 1 >= stop_orders) {
					closeAllAndStop(1);
					return;
				}
	...
	public void onTick(Instrument instrument, ITick tick) throws JFException {
		if(instrument.equals(this.instrument)) {
			...
			/*
			 * if count profit > target then closeAll and stop old_strategy
			 */
			if(countProfit(gridsNormalAsk.getAllOrders()) + countProfit(gridsNormalBid.getAllOrders()) >= stop_profit) {
				closeAllAndStop(2);
				return;
			}
	 ...
	 * 平所有头寸并退出策略
	 */
	public void closeAllAndStop(int type) {
		...
		log("==== close all and stop / " + reason + " ====");
		closeOrder(gridsNormalAsk.getAllOrders());
		closeOrder(gridsNormalBid.getAllOrders());
		context.stop();
	}
